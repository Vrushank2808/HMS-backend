...
const createTransporter = () => {
    return nodemailer.createTransport({
        ...
        rateLimit: 3, // Rate limiting: 3 emails per rateDelta
...
const sendOTPEmail = async (email, otp, userName) => {
    ...
    return Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('OTP email sending timeout after 25 seconds')), timeout)
        )
    ]).catch(error => {
        console.error('❌ OTP email sending failed or timed out:', error.message);
        return { success: false, error: error.message };
    });
};
...
const sendCredentialsEmail = async (email, credentials, role) => {
    ...
    return Promise.race([
        ...
        // Timeout promise
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Credentials email sending timeout after 25 seconds')), timeout)
        )
    ]).catch(error => {
        console.error('❌ Credentials email sending failed or timed out:', error.message);
        return { success: false, error: error.message };
    });
};
